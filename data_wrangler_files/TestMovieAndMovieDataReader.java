...
public class TestMovieAndMovieDataReader {
	...
	/**
	 * This test reads 3 movies and checks if the getGenres() returns the expected array list for each of the movies. 
	 */ 
	public boolean testReaderGetGenres() {
		List<MovieInterface> movieList;
		try {
			movieList = readerToTest.readDataSet(new StringReader(
					"title,original_title,year,genre,duration,country,language,director,writer,production_company,actors,description,avg_vote\n"
	...
	/**
	 * This tests the getDirector() method and checks if the directors of the 3 movies match the expected values.
	 */	
	public boolean testReaderGetDirector() {
		List<MovieInterface> movieList;
		try {
			movieList = readerToTest.readDataSet(new StringReader(
					"title,original_title,year,genre,duration,country,language,director,writer,production_company,actors,description,avg_vote\n"
		...
		String[] expectedDirectors = new String[]{ "Ryan Bury, Jennifer Bonior", "Rene Perez", "Rachel Lee Goldenberg" };
		for(int i = 0; i < expectedDirectors.length; i++) {
			if(!expectedDirectors[i].equals(movieList.get(i).getDirector())) {
				// test failed
				return false;
			}
		}	
